// src/components/SpecialistDetails.jsx
import { useState } from 'react';
import { useParams, Link } from 'react-router-dom';
import specialists from './data';
import pin from './pin-map.png';
...
const SpecialistDetails = () => {
  ...
  const hasPhotos = specialist.photos && specialist.photos.length > 0;
  const hasMultiplePhotos = hasPhotos && specialist.photos.length > 1;

  const nextImage = () => {
    if (hasPhotos && currentImageIndex < specialist.photos.length - 1) {
  ...
  const prevImage = () => {
    if (hasPhotos && currentImageIndex > 0) {
      ...
      setCurrentImageIndex(specialist.photos.length - 1);
    }
  };
      ...
      <div className="specialist-details">
        ...
        <div className="gallery-contacts">
          {hasPhotos ? (
            <div className="gallery">
              <div className="image-container">
                ...
                <img
                  ...
                  src={specialist.photos[currentImageIndex]}
                  alt={`${specialist.name} фото`}
                />
                {hasMultiplePhotos && (
            ...
            <p>Галерея не доступна.</p>
          )}

          <div className="contacts">
            <h3>Адрес:</h3>
            <p className="address-container">
              <img src={pin} alt="Address Icon" width="30" />
              {specialist.address}
            </p>
            <h3>Как связаться:</h3>
            <div className="contact-buttons">
              <button
                className={`contact-button ${specialist.telegramChannel ? 'active' : 'inactive'}`}
                onClick={() =>
                  specialist.telegramChannel && window.open(specialist.telegramChannel, '_blank')
                }
                disabled={!specialist.telegramChannel}
              >
                <svg
              ...
              <button
                className={`contact-button ${specialist.instagram ? '' : 'disabled'}`}
                onClick={() => specialist.instagram && window.open(specialist.instagram, '_blank')}
                disabled={!specialist.instagram}
              >
                <svg
                  className="networkIcon"
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <button
                className={`contact-button ${specialist.contact ? '' : 'disabled'}`}
                onClick={() =>
                  specialist.contact && window.open(`https://wa.me/${specialist.contact}`, '_blank')
                }
                disabled={!specialist.contact}
              >
                <svg
                  className="networkIcon"
                  xmlns="http://www.w3.org/2000/svg"
            ...
            <button
              className={`contact-website ${specialist.website ? 'active' : 'inactive'}`}
        ...
        {specialist.price && specialist.price.length > 0 && (
          <div className="specialist-price">
            <h3>Прайс</h3>
            <ul>
              {specialist.price.map((item, index) => (
                <li key={index}>
                  <span className="menu-item">{item.name}</span>
                  <span className="menu-price">{item.price} BRL</span>
                </li>
              ))}
            </ul>
          </div>
